...
def calculate_surface_brightness_profile_numpy(data, center=None, max_radius=None, 
                                             bin_width=1.0, background_subtract=True):
    """
    Calculate radial surface brightness profile using numpy operations.
    This avoids issues with photutils aperture classes.
    ...
    bin_width : float, optional
        Width of radial bins in pixels (default: 1.0)
    ...
    if background_subtract:
        # Use pixels beyond 80% of max_radius for background estimation
    ...
    for i in range(num_bins):
        ...
        # Create mask for this annular bin
        if r_inner == 0:
            # For the central bin, use a circular mask
            mask = distance <= r_outer
        else:
            # For other bins, use an annular mask
            mask = (distance > r_inner) & (distance <= r_outer)
        
        if np.sum(mask) > 0:  # Only process if there are pixels in this bin
            pixels_in_bin = data_corrected[mask]
            
            # Calculate surface brightness statistics
            mean_sb = np.mean(pixels_in_bin)
            total_flux = np.sum(pixels_in_bin)
            n_pixels = len(pixels_in_bin)
            
            # Estimate uncertainty (combination of Poisson and pixel-to-pixel variation)
            if total_flux > 0:
                poisson_err = np.sqrt(np.abs(total_flux)) / n_pixels
                pixel_std = np.std(pixels_in_bin) / np.sqrt(n_pixels)
                total_err = np.sqrt(poisson_err**2 + pixel_std**2)
            else:
                total_err = np.std(pixels_in_bin) / np.sqrt(n_pixels) if n_pixels > 1 else 0
            
            radii.append(r_center)
            surface_brightness.append(mean_sb)
            brightness_err.append(total_err)
...
def plot_surface_brightness_profile(radii, surface_brightness, brightness_err,
    ...
    if magnitude_scale:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            mag_sb = -2.5 * np.log10(np.abs(surface_brightness))
            mag_err = 2.5 * brightness_err / (np.abs(surface_brightness) * np.log(10))
        
        # Filter out invalid values
        valid_mask = np.isfinite(mag_sb) & np.isfinite(mag_err) & (surface_brightness > 0)
...
Examples:
  ...
  python fits_profile.py image.fits --magnitude --save profile.png
    ...
    parser.add_argument('--bin-width', type=float, default=1.0,
                       help='Width of radial bins in pixels (default: 1.0)')
    ...
    # Calculate surface brightness profile using numpy method
    print("Calculating surface brightness profile...")
    radii, sb, sb_err = calculate_surface_brightness_profile_numpy(
        data, 
        center=center,
